...
export default function PendingOrganizersPage() {
    ...
    const handleApprove = async (organizer: PendingOrganizer) => {
        if (!organizer.password) {
            alert("Error: Password not found for this user.");
            return;
        }
        ...
        try {
            await createUserWithEmailAndPassword(adminAuth, organizer.email, organizer.password);
    ...
    const handleDeny = async (organizerId: string) => {
        ...
        try {
            ...
            alert(`Error denying organizer: ${error.message}`);
        } finally {
    ...
    return (
        <div className="space-y-6 p-8">
            ...
            <Card className="border-zinc-200 bg-white shadow-lg">
                <CardHeader>
                    <CardTitle className="text-zinc-900">Pending Requests ({pendingOrganizers.length})</CardTitle>
                </CardHeader>
                <CardContent>
                    {pendingOrganizers.length === 0 ? (
                        <p className="text-center text-zinc-500">No pending organizer requests.</p>
                    ) : (
