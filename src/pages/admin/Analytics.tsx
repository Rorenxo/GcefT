...
type StudentDoc = {name?: string
    department?: string
    [k: string]: any
}

type EventDoc = {title?: string
    department?: string
    organizerId?: string
    [k: string]: any
}

type OrganizerDoc = {name?: string
    [k: string]: any
}

const SummaryCard: React.FC<{title: string
    value: number | string
...
const eventsPerDepartment = useMemo(() => {
    ...
    const arr = Array.from(map.entries()).map(([department, count]) => ({ department, count }))
...
const organizerActivity = useMemo(() => {
    const map = new Map<string, number>()
    events.forEach((ev) => {
    const id = ev.organizerId ?? "unknown"
    map.set(id, (map.get(id) ?? 0) + 1)
    })
    const arr = Array.from(map.entries()).map(([organizerId, count]) => {
    const org = organizers.find((o: any) => (o.id ?? "") === organizerId)
    const name = org?.name ?? organizerId
    return { organizerId, name, count }
    })
    arr.sort((a, b) => b.count - a.count)
    return arr
}, [events, organizers])
...
const deptChartData = useMemo(() => {
    if (eventsPerDepartment.length === 0) return [{ department: "No data", count: 0 }]
    return eventsPerDepartment.map((d) => ({ department: d.department, count: d.count }))
}, [eventsPerDepartment])
...
const handleDownloadPDF = async () => {
    ...
    pdf.save("GCEF_Analytics_Report.pdf", { returnPromise: true })
}
    ...
    return (
        ...
        <div ref={dashboardRef} id="dashboard-export" className="space-y-6">
            ...
            <SummaryCard title="Total Students" value={loading ? "…" : totalStudents} icon={<Users className="h-6 w-6 text-zinc-400" />} />
            <SummaryCard title="Total Events" value={loading ? "…" : totalEvents} icon={<Calendar className="h-6 w-6 text-zinc-400" />} />
            <SummaryCard title="Total Organizers" value={loading ? "…" : totalOrganizers} icon={<CreditCard className="h-6 w-6 text-zinc-400" />} />
            </div>
            ...
            >
                ...
                {deptChartData.map((d: any, i: number) => (
                    <div key={d.department} className="flex items-center gap-2 text-sm text-zinc-600">
                    <span
                        className="inline-block h-3 w-3 rounded-sm"
                        style={{
                        background:
                            DEPARTMENT_COLORS[d.department] ||
                            DEFAULT_COLORS[i % DEFAULT_COLORS.length],
                        }}
                    />
                    <span>{d.department}</span>
                    <span className="ml-1 text-zinc-400">({d.count})</span>
                    </div>
                ))}
                </div>
            </motion.div>
            ...
            >
                ...
                <div className="text-xs text-zinc-500">Top organizers by posted events</div>
                </div>
                    ...
                    {organizerActivity.length === 0 ? (
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{
                                background: DEFAULT_COLORS[idx % DEFAULT_COLORS.length],
                                }}
                            >
                                {o.name?.slice(0, 2).toUpperCase()}
                            </div>
                            <div className="min-w-0">
                                <p className="font-medium text-sm truncate mb-1.5">{o.name}</p>
